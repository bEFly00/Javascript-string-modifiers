...
function CharAt() {
    ...
    let charAt = document.getElementById('charat').value;
    let adottKarakter = text.charAt(charAt - 1);
...
function Includes() {
    ...
    document.getElementById('eredmeny').value = adottKarakter
        ? "Az eredeti szöveg tartalmazza a megadott szövegrészt"
        : "Az eredeti szöveg nem tartalmazza a megadott szövegrészt";
}
...
function Camelcase() {
    ...
    for (let i = 0; i < daraboltSzoveg.length; i++) {
        ...
        camelCaseSzoveg += i == 0
            ? daraboltSzoveg[0].toLowerCase()
            : Capitalize(daraboltSzoveg[i])
    }
